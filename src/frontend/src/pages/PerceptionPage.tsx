...
const STATUS_STYLES: Record<string, string> = {
  ...
  Completed: 'bg-[var(--badge-bg)] text-gray-500 border border-gray-700',
  ...
  return (
    <div className="p-6 h-full flex flex-col gap-6 overflow-hidden">
      ...
      <section className="flex-1 flex gap-6 min-h-[250px]">
        ...
        <div className="w-1/3 flex flex-col gap-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-[10px] font-mono text-gray-500 bg-[var(--badge-bg)] px-1.5 rounded">1080p | 30fps</span>
          </div>
          <VisualFeed />
        </div>
            ...
            <div className="flex flex-col gap-3">
              {asrLog.map((e, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div className="flex flex-col">
                    <span className={`text-xs font-bold mb-0.5 ${e.role === '用户' || e.role === '语音' ? 'text-sky-400' : e.role === 'AI' ? 'text-emerald-400' : 'text-amber-400'}`}>[{e.role}]</span>
              ...
              {asrLog.length === 0 && (
                <div className="flex gap-3 border-l-2 border-[var(--accent-blue)] pl-3 bg-[var(--accent-blue)]/5 py-2 rounded-r">
                  ...
                  <div><span className="text-gray-400 text-xs">监听中...</span><br /><span className="text-[var(--text-primary)] animate-pulse">...</span></div>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="h-px w-full bg-gray-700/50 relative flex items-center justify-center">
        <div className="bg-[var(--panel-bg)] px-4 text-xs font-mono text-gray-500">层间接口</div>
      </div>
          ...
          <div className="flex gap-2">
            {MOTOR_FILTERS.map(f => (
              <button key={f} onClick={() => setMotorFilter(f)}
                className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${motorFilter === f ? 'bg-[var(--accent-blue)] text-white shadow-lg shadow-[var(--accent-blue)]/20' : 'bg-[var(--badge-bg)] text-gray-400 border border-gray-700 hover:border-gray-500'}`}>
                {f}
              </button>
            ))}
          </div>
...
function StatusBadge({ icon, label, online }: { icon: string; label: string; online: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded bg-[var(--badge-bg)] border border-gray-700">
...
function VisualFeed() {
  ...
  useEffect(() => {
    api.get('/config').then(r => {
      const on = r.data?.action?.screen?.enabled !== false
      setEnabled(on)
    }).catch(() => {})
  }, [])
...
function MotorTable({ motors }: { motors: MotorEntry[] }) {
  return (
    <div className="flex-1 bg-[var(--badge-bg)] rounded-xl border border-gray-700/50 overflow-hidden flex flex-col">
      <div className="grid grid-cols-12 gap-4 px-4 py-3 border-b border-gray-700/50 bg-[var(--panel-bg-alt)] text-xs font-medium text-gray-400 uppercase tracking-wider">
