  ...
  return (
    <div className="glass-panel rounded-xl p-5 flex flex-col gap-4 h-full">
      ...
      {status && (!status.services_ready || hasError) && (
        <div className="flex flex-wrap gap-2 -mt-2">
          {Object.entries(status.loading_status ?? {}).map(([k, v]) => (
            <div key={k} className="flex items-center gap-1.5 px-2 py-1 rounded bg-black/20 border border-gray-700/30 text-[11px] font-mono">
      ...
      <div className="grid grid-cols-2 gap-4 flex-1">
        {/* GPU */}
        <div className="bg-black/20 rounded-lg p-3 border border-gray-700/30 flex flex-col justify-between">
          <div>
